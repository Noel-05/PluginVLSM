    ...
    try {
        ...
        if (typeof err == "string")
            ...
            // *
                ...
                clfDiv.innerHTML =
                    "<p class='alarming-text'>INTERNAL ERROR</p>";
        else
            clfDiv.innerHTML =
                "<p class='alarming-text'>INTERNAL ERROR</p>";
    }
...
clsForm.addEventListener("submit", (e) => {
    ...
    try {
        ...
        clsClassLvlNeighborsDiv.innerHTML =
            `<p>Todas las /${prefix} subredes posibles en ${
                classNeighboringSubnetsList[0].subnetIp
            }/${8 * parseInt(prefix / 8)} con sus detalles</p>` +
        ...
        clsUpperPrefixesDiv.innerHTML =
            `<p>Posibles /${prefix} subredes vecinas en una variedad de prefijos más grandes</p>` +
        ...
        if (typeof err == "string")
            ...
            //*
                ...
                clsClassLvlNeighborsDiv.innerHTML =
                    "<p class='notice-text'>Estos campos de información no estan disponibles para /8 /16 /24 subredes</p>";
                clsUpperPrefixesDiv.innerHTML =
                    "<p class='notice-text'>Estos campos de información no estan disponibles para /8 /16 /24 subredes</p>";
            }
            //*
                ...
                clsUpperPrefixesDiv.innerHTML =
                    "<p class='notice-text'>Estos campos de información no estan disponibles para /9  /17 /25 subredes</p>";
            //*
            // thrown from functions
                ...
                clsDiv.innerHTML = "<p class='alarming-text'>" + err + "</p>";
            else if (err.includes("máscara inválida"))
                clsDiv.innerHTML = "<p class='alarming-text'>" + err + "</p>";
            // *
            else
                clsDiv.innerHTML =
...
vlsmForm.addEventListener("submit", (e) => {
    ...
    try {
        ...
        if (neededSize > mainSubnet.size)
            throw `Se excedió la capacidad de la subred principal con ${
        ...
        if (typeof err == "string")
            if (err.includes("anti-vlsm prefix"))
                // thrown from main scope
                vlsmChunksDiv.innerHTML =
                    "<p class='alarming-text'>VLSM no puede ser aplicada en /32, /31 and /30 subredes</p>";
            //*
            ...
            //*
            // thrown from functions
            else if (err == "La notación CIDR debe incluir el carácter /")
                vlsmChunksDiv.innerHTML =
            ...
            //*
                ...
                vlsmChunksDiv.innerHTML =
                    "<p class='alarming-text'>" + err + " </p>";
            //*
            else if (err.includes("Dirección de red inválida"))
                vlsmChunksDiv.innerHTML =
                    "<p class='alarming-text'>" + err + " </p>";
            //*
            else
...
function ipv4MaskValidity(mask) {
    ...
    if (mask.length !== 4) throw `máscara inválida ${mask}`;

    mask.forEach((octet) => {
        if (!maskDecimals.includes(parseInt(octet)))
            if (octet != 0) throw `máscara inválida ${mask}`;
    });
}
...
function netAddrValidty(ip, intrestOctInd, blockSize, prefix) {
    if (ip[intrestOctInd] % blockSize !== 0)
        throw `Dirección de red inválida ${ip.join(".")} para /${prefix} prfix`;
    for (let i = 3; i > intrestOctInd; i--) {
        if (ip[i] != 0)
            throw `Dirección de red inválida ${ip.join(
                "."
            )} para /${prefix} prfix`;
    }
}
...
function cidrToSubnetAndPrefix(cidr) {
    ...
    else throw "La notación CIDR debe incluir el carácter /";
...
function subnetTableGen(info) {
    ...
    <table class='properties-table'>
        <tr>
            <th>Dirección de ${info.ipClass == undefined ? "subred" : "red"}</th>
            <td>${info.subnetIp}</td>
            </tr>
        <tr>
            <th>Primer host</th>
        ...
        ${
            info.ipClass == undefined
                ? ""
                : `<tr><th>Clase</th><td>${info.ipClass}</td></tr>`
        }
        <tr>
            <th>Número de hosts disponibles</th>
...
function getClassNeighboringSubnets(mainSubnet, mask, intrestOctInd) {
    if (mask[intrestOctInd] == "255")
        return "La subred principal es una subred de nivel de clase";
